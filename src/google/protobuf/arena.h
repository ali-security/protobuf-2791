...
class PROTOBUF_EXPORT PROTOBUF_ALIGNAS(8)
 ...
 public:
  ...
  template <typename T>
  class ABSL_ATTRIBUTE_TRIVIAL_ABI ABSL_NULLABILITY_COMPATIBLE UniquePtr;

  // A smart pointer type for holding objects that are statically known to be
  // owned by an `Arena`.
  // `Ptr<T>` is similar to `T*` except it holds extra static information (the
  // fact that it is arena owned) and extra dynamic information (the arena that
  // owns it).
  // Main differences from `UniquePtr` are:
  ...
  template <typename T>
  class ABSL_ATTRIBUTE_TRIVIAL_ABI Ptr;
  ...
  // Create a message of the same type as `prototype`, maybe on an `Arena`,
  // returned in an `UniquePtr`.
  // This calls `prototype.New(arena)`. Similar to calling
  // `MakeUnique<T>(arena)`, but works when the runtime type of the object is
  // unknown and you only have a `Message` or `MessageLite` prototype.
  template <int&..., typename Prototype>
  [[nodiscard]] PROTOBUF_NDEBUG_INLINE static UniquePtr<Prototype>
      PROTOBUF_NONNULL MakeUniqueFromPrototype(Arena* PROTOBUF_NULLABLE arena,
                                               const Prototype& prototype) {
    static_assert(std::is_base_of_v<MessageLite, Prototype>);
    return UnsafeWrapUniquePtr(arena, prototype.New(arena));
  }
...
template <typename T>
class ABSL_ATTRIBUTE_TRIVIAL_ABI ABSL_NULLABILITY_COMPATIBLE Arena::UniquePtr
 ...
 private:
  ...
  // nullptr. The owning_arena should be the arena (or null) that was used to
  // create element being maybe deleted. Also provides a non-const pointer to
  // the owning arena via GetOwningArena().
  struct Deleter {
 ...
 public:
  using pointer = typename UniquePtrType::pointer;
  using element_type = typename UniquePtrType::element_type;
  ...
  constexpr UniquePtr(UniquePtr<U>&& rhs)
      : ptr_(rhs.release(), Deleter{rhs.GetOwningArena()}) {}
...
template <typename T>
class ABSL_ATTRIBUTE_TRIVIAL_ABI Arena::Ptr : internal::ArenaPtrCmpBase {
 public:
  using pointer = T*;
