...
struct RepeatedFieldTraits {
  ...
  static void SetElement(ElementType& element, ElementType value) {
    element = value;
  }
};
...
struct RepeatedFieldTraits<ElementType,
  ...
  template <typename T>
  static void SetElement(ElementType& element, T&& value) {
...
struct RepeatedFieldTraits<absl::string_view> {
  ...
  template <typename T>
  static void SetElement(std::string& element, T&& value) {
    string_util::SetElement(element, std::forward<T>(value));
  }
};
...
struct RepeatedFieldTraits<absl::Cord> {
  ...
  static void SetElement(absl::Cord& element, absl::Cord&& value) {
    element = std::move(value);
  }
  static void SetElement(absl::Cord& element, absl::string_view value) {
    element = value;
  }
};
...
class RepeatedFieldProxy final
 ...
 public:
  ...
  //
  // Prefer to `std::move` values passed to this method to avoid deep copies,
  // especially for message and `std::string` types.
  //
  // `index` must be less than `size()`, otherwise behavior is undefined.
  template <typename T>
  void set(size_type index, T value) const {
    internal::RepeatedFieldTraits<ElementType>::SetElement(
        Base::field()[index], std::forward<T>(value));
  }
