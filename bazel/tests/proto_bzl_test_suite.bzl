...
def bzl_test_suite(
    ...
    for (impl, target) in tests:
        test_name = create_test_name(impl, name)
        analysis_test(
            name = test_name,
            target = target,
            impl = impl,
...
def create_test_name(impl, name):
    # Starlark currently stringifies a function as "<function NAME>", so we use
    # that knowledge to parse the "NAME" portion out.
    impl_name = str(impl).partition("<function ")[2].partition(" ")[0].partition(">")[0]
    if impl_name.startswith("_"):
        impl_name = impl_name[1:]
    return impl_name + "_" + name
